 ...
 * Tests for Change Scenario: ('M', 12) -> put('F', 12) -> ('M', 12), ('F', 12)
 * with alpha = 0.75, capacity = 13, using double hashing.  
 * @author CS 321
 */

public class HashTableTest_12t_M12_putF12_M12F12_75_13_double
{
	...
	private static final Character FIRST_VALUE = TestCase.M12.getValue(); 
	private static final int FIRST_KEY = TestCase.M12.getKey(); 
	private static final Character SECOND_VALUE = TestCase.F12.getValue(); 
	private static final int SECOND_KEY = TestCase.F12.getKey(); 
	// Valid value / key pairs 
	private static final Object[][] VALID_VALUES_KEYS = { {TestCase.M12.getValue(), TestCase.M12.getKey() }, 
			{TestCase.F12.getValue(), TestCase.F12.getKey()} };
	// Expected hashes for given keys and attempts 
	private static final Object[][] EXPECTED_HASHES = { {TestCase.M12.getKey(), 0, 12}, {TestCase.M12.getKey(), 1, 1}, 
			{TestCase.M12.getKey(), 2, 3}, {TestCase.M12.getKey(), 3, 5},{TestCase.M12.getKey(), 4, 7}, 
			{TestCase.M12.getKey(), 5, 9}, {TestCase.M12.getKey(), 6, 11},{TestCase.M12.getKey(), 7, 0}, 
			{TestCase.M12.getKey(), 8, 2},{TestCase.M12.getKey(), 9, 4}, {TestCase.M12.getKey(), 10, 6},
			{TestCase.M12.getKey(), 11, 8}, {TestCase.M12.getKey(), 12, 10}, {TestCase.M12.getKey(), 13, 12}, 
			{TestCase.F12.getKey(), 0, 12}, {TestCase.F12.getKey(), 1, 1}, 
			{TestCase.F12.getKey(), 2, 3}, {TestCase.F12.getKey(), 3, 5},{TestCase.F12.getKey(), 4, 7}, 
			{TestCase.F12.getKey(), 5, 9}, {TestCase.F12.getKey(), 6, 11},{TestCase.F12.getKey(), 7, 0}, 
			{TestCase.F12.getKey(), 8, 2},{TestCase.F12.getKey(), 9, 4}, {TestCase.F12.getKey(), 10, 6},
			{TestCase.F12.getKey(), 11, 8}, {TestCase.F12.getKey(), 12, 10}, {TestCase.F12.getKey(), 13, 12} }; 
	// Valid value / key pairs with frequencies 
	private static final Object[][] VALID_FREQUENCIES = { {TestCase.M12.getValue(), TestCase.M12.getKey(), 0}, 
			{TestCase.F12.getValue(), TestCase.F12.getKey(), 0} };	
