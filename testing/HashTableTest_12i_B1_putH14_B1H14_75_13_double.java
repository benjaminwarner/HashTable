 ...
 * Tests for Change Scenario: ('B', 1) -> put('H', 14) -> ('B', 1), ('H', 14)
 * with alpha = 0.75, capacity = 13, using double hashing.  
 * @author CS 321
 */

public class HashTableTest_12i_B1_putH14_B1H14_75_13_double
{
	...
	private static final Character FIRST_VALUE = TestCase.B1.getValue(); 
	private static final int FIRST_KEY = TestCase.B1.getKey(); 
	private static final Character SECOND_VALUE = TestCase.H14.getValue(); 
	private static final int SECOND_KEY = TestCase.H14.getKey(); 
	// Valid value / key pairs 
	private static final Object[][] VALID_VALUES_KEYS = { {TestCase.B1.getValue(), TestCase.B1.getKey() }, 
			{TestCase.H14.getValue(), TestCase.H14.getKey()} };
	// Expected hashes for given keys and attempts 
	private static final Object[][] EXPECTED_HASHES = { {TestCase.B1.getKey(), 0, 1}, {TestCase.B1.getKey(), 1, 3}, 
			{TestCase.B1.getKey(), 2, 5}, {TestCase.B1.getKey(), 3, 7},{TestCase.B1.getKey(), 4, 9}, 
			{TestCase.B1.getKey(), 5, 11}, {TestCase.B1.getKey(), 6, 0},{TestCase.B1.getKey(), 7, 2}, 
			{TestCase.B1.getKey(), 8, 4},{TestCase.B1.getKey(), 9, 6}, {TestCase.B1.getKey(), 10, 8},
			{TestCase.B1.getKey(), 11, 10}, {TestCase.B1.getKey(), 12, 12}, {TestCase.B1.getKey(), 13, 1}, 
			{TestCase.H14.getKey(), 0, 1}, {TestCase.H14.getKey(), 1, 5}, 
			{TestCase.H14.getKey(), 2, 9}, {TestCase.H14.getKey(), 3, 0},{TestCase.H14.getKey(), 4, 4}, 
			{TestCase.H14.getKey(), 5, 8}, {TestCase.H14.getKey(), 6, 12},{TestCase.H14.getKey(), 7, 3}, 
			{TestCase.H14.getKey(), 8, 7},{TestCase.H14.getKey(), 9, 11}, {TestCase.H14.getKey(), 10, 2},
			{TestCase.H14.getKey(), 11, 6}, {TestCase.H14.getKey(), 12, 10}, {TestCase.H14.getKey(), 13, 1} }; 
	// Valid value / key pairs with frequencies 
	private static final Object[][] VALID_FREQUENCIES = { {TestCase.B1.getValue(), TestCase.B1.getKey(), 0}, 
			{TestCase.H14.getValue(), TestCase.H14.getKey(), 0} };	
